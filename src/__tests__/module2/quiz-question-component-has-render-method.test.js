...
describe('InvoiceEntry Component', () => {
  it('has a render method that returns the correct HTML  @quiz-question-component-has-render-method', () => {
    ...
    assert(div.querySelectorAll('main section') != null, "We can't find a `main` tag with a child of `section` in the InvoiceEntry component's JSX.")
    ...
    assert(div.querySelectorAll('main section')[0].querySelector('p'), "We can't find a `section` tag with a child of `p` in the InvoiceEntry component's JSX.")
    assert(div.querySelectorAll('main section')[1].querySelector('ul'), "We can't find a `ul` tag with a child of `li` in the InvoiceEntry component's JSX.")
  })
})